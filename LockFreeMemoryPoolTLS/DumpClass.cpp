	...
	LONG WINAPI MemoryDump::MyExceptionFilter(__in PEXCEPTION_POINTERS pExceptionPointer)
	{
		...
		if (dumpCount == 1)
		{
			...
			if (dumpFile != INVALID_HANDLE_VALUE)
			{
				...
				bool ret =MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), dumpFile, MiniDumpWithFullMemory, &dumpInfo, nullptr, nullptr);
				if (ret == false)
				{
					printf("MiniDumpWriteDump function return false\n");
				}
				CloseHandle(dumpFile);
				printf("Dump Saved Clear\n");
			}
			else
			{
				wprintf(L"dumpFile INVALID_HANDLE_VALUE err : %d\n", GetLastError());
			}
		}
