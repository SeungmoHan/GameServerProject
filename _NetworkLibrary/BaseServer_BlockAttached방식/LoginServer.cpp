...
namespace univ_dev
{
    ...
    unsigned __stdcall LoginMoniteringThread(void* param)
    {
    ...
    void LoginServer::Start()
    {
        ...
        this->_MoniteringThread = (HANDLE)_beginthreadex(nullptr, 0, LoginMoniteringThread, this, 0, nullptr);
        ...
        HANDLE threads[]{ _RedisThread,_MoniteringThread };
    ...
    LoginServer::LoginServer(USHORT port, DWORD backlogQueueSize, DWORD threadPoolSize, DWORD runningThread, DWORD nagleOff, ULONGLONG maxSessionCounts, DWORD timeOutTime)
    {
        Start();
    }
    ...
    unsigned int LoginServer::LoginServerMoniteringThread(void* param)
    {
        ...
        while (this->_RunningFlag)
        {
            ...
            printf("|----------------------------------------TPS-------------------------------------------\n");
            printf("| Accept Per Sec : %llu\n", info._TotalAcceptSessionCount - lastTotalAccept);
            printf("| TPS// Recv / Send / Total  : %llu / %llu / %llu\n", info._TotalRecvPacketCount, info._TotalSendPacketCount, info._TotalSendPacketCount+ info._TotalRecvPacketCount);
    ...
    void LoginServer::OnErrorOccured(DWORD errorCode, const WCHAR* error,LogClass::LogLevel level = LogClass::LogLevel::LOG_LEVEL_LIBRARY)
    {
        //this->GetLastAPIErrno();
        //this->GetLastCoreErrno();
        this->_LoginServerLog.LOG(error, level);
    }
