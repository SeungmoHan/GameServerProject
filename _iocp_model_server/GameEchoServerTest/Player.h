...
namespace univ_dev
{
	struct Player
	{
		Player() : _Logined(0), _SessionID(0), _AccountNo(0), _SectorX(-1), _SectorY(-1), _ID{ 0 }, _NickName{ 0 }, _TokenKey{ 0 }, _LastAction{ 0 },_ThreadBlockName(""){};
		Player(ULONGLONG sessionID, INT64 accountNo, WORD sectorX, WORD sectorY, const WCHAR* ID, const WCHAR* nickname)
			:_SessionID(sessionID), _AccountNo(accountNo), _SectorX(sectorX), _SectorY(sectorY), _TokenKey{ 0 }, _LastAction{ 0 },_ThreadBlockName("")
		{
