...
/// 1초 측정 타이머 v
BYTE g_TileMap[MAP_HEIGHT][MAP_WIDTH];
univ_dev::PlayerObject* gp_Player;
univ_dev::CameraObject* gp_MainCamera = new univ_dev::CameraObject();
...
#else
    addr.sin_port = htons(TCP_FIGHTER_PORT);
#endif
...
bool ReadEvent()
{
    ...
    if (g_RecvRingBuffer.GetUseSize() == 0)
...
namespace univ_dev
{
    ...
    void PacketProcCreateMyPlayer(Packet& rawPacket)
    {
        ...
        myPlayer->SetActionStand();
        gp_Player = myPlayer;
        gp_MainCamera->SetTarget(gp_Player);
        g_ObjectList.push_back(gp_MainCamera);
        g_ObjectList.push_back(myPlayer);
    }
...
void Render()
{
    ...
    univ_dev::g_SpriteDibBuffer.DrawImage((int)univ_dev::e_SPRITE::MAP, 0, 0, dibBuffer, width, height, pitch);
    //카메라의 현재 좌표를 가지고 찍어줘야된다...
    //플레이어는 플레이어위치 - 카메라의 위치 한 위치에 찍혀야된다.
    //if (gp_MainCamera != nullptr)
    //    printf("Camera X Pos : %d\tCamera Y Pos : %d\n", gp_MainCamera->GetCurrentX(), gp_MainCamera->GetCurrentY());
    //if (gp_Player != nullptr)
    //    printf("Player X Pos : %d\tPlayer Y Pos : %d\n\n", gp_Player->GetCurrentX(), gp_Player->GetCurrentY());

    printf("camera object + user player + other player : num of Object : %llu\n", univ_dev::g_ObjectList.size());
    LayerOrdering();
    int worldObjectYPos;
    int worldObjectXPos;

    int cameraXPos = gp_MainCamera->GetCurrentX();
    int cameraYPos = gp_MainCamera->GetCurrentY();
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        for (int j = 0; j < 11; j++)
        {
            univ_dev::g_SpriteDibBuffer.DrawSprite(g_TileMap[tileYIndex][tileXIndex++], j * 64 - tileXOverFlow, i * 64 - tileYOverFlow, dibBuffer, width, height, pitch);
        }
        tileYIndex++;
    }
    ...
    for (__int64 i = 0; i < univ_dev::g_ObjectList.size(); i++)
    {
        univ_dev::BaseObject* currentObject = univ_dev::g_ObjectList[i];
        int tileIDX = 
        worldObjectXPos = univ_dev::g_ObjectList[i]->GetCurrentX();
        worldObjectYPos = univ_dev::g_ObjectList[i]->GetCurrentY();

        screenObjectXPos = worldObjectXPos - cameraXPos;
        screenObjectYPos = worldObjectYPos - cameraYPos;
        currentObject->SetPosition(screenObjectXPos, screenObjectYPos);
        currentObject->Render(dibBuffer, width, height, pitch);
        currentObject->SetPosition(worldObjectXPos, worldObjectYPos);
    }
...
bool InitialGame()
{
    ...
    if (!univ_dev::g_SpriteDibBuffer.LoadDibSprite((int)univ_dev::e_SPRITE::TILE_MAP, L"Sprite_Data/Tile_01.bmp", 0, 0))return false;
    
    memset(g_TileMap, (BYTE)univ_dev::e_SPRITE::TILE_MAP, MAP_WIDTH * MAP_HEIGHT);
